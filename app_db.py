...
The app reads the dbt-built marts straight out of ``vegas.duckdb`` (materialized
by ``build_warehouse.py`` + ``dbt build``). No Snowflake, no CSV snapshot — the
dbt output *is* what the app queries.
"""
